...
import { Download, Trash2, CheckCircle2, Clock, ChefHat } from 'lucide-react';
...
export default function Staff() {
  ...
  const clearAllOrders = useAppStore(state => state.clearAllOrders);
  ...
  const newOrdersCount = todayOrders.filter(o => o.status === 'baru').length;
  ...
  const exportCSV = () => {
    ...
    orders.forEach(o => {
      ...
      csv += `${o.id},"${date}",${o.table},${o.status},${o.paymentMethod},${o.total},"${itemsStr}"\n`;
    });
      ...
      <div className="max-w-6xl mx-auto space-y-8">
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    Tindakan ini tidak dapat dibatalkan. Semua riwayat pesanan akan dihapus dari perangkat ini.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-border">
            ...
            <div className="text-4xl font-bold text-primary">{newOrdersCount}</div>
          </div>
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-border">
            <div className="text-sm font-medium text-muted-foreground mb-2">Total Pendapatan (Hari Ini)</div>
            <div className="text-4xl font-bold text-foreground">{formatRupiah(totalRevenue)}</div>
          </div>
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-border">
                ...
                {filteredOrders.length === 0 ? (
                  ...
                  filteredOrders.map(order => (
                    <TableRow key={order.id}>
                      <TableCell>
                        <div className="font-mono font-medium text-primary">{order.id}</div>
                        <div className="text-xs text-muted-foreground">
                          {new Date(order.createdAt).toLocaleTimeString('id-ID', { hour: '2-digit', minute:'2-digit' })}
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="font-bold">{order.table}</div>
                      </TableCell>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          {order.status === 'baru' && (
                            <Button size="sm" onClick={() => updateOrderStatus(order.id, 'disiapkan')}>
                              <ChefHat className="w-4 h-4 mr-1" /> Siapkan
                            </Button>
                          )}
                          {order.status === 'disiapkan' && (
                            <Button size="sm" variant="secondary" className="bg-green-100 text-green-700 hover:bg-green-200" onClick={() => updateOrderStatus(order.id, 'selesai')}>
